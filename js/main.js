...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    //Here repalce all querySelector by getElementById
    switch(size) {
      case "1":
  ...
  // offsetWidth. delete the determinDex function since we are not using that with getElementsByClassName, but keep the sizeSwitcher
  ...
  function changePizzaSizes(size) {
    var numberOfPizzas = document.getElementsByClassName("randomPizzaContainer");
    //get the current width, here I am using offsetWidth once to opmtimize my code
    var currentWidth = document.getElementById("randomPizzas").offsetWidth;
    //Instead of using querySelector, get the element by id to increase the performance
    for (var i = 0; i < numberOfPizzas.length; i++) {
        numberOfPizzas[i].style.width = (sizeSwitcher(size) * currentWidth) + 'px';
    }
  }
...
//Move the pizzaDiv outside the loop
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < slidPizzas; i++) {
    ...
    //delete the queryselector and call the element by getElementById 
